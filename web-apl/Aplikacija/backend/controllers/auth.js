...
export const login = async (req, res) => {
  try {
    ...
    if (user.tipKorisnika == "Stanar") {
      ...
      const saradnik = await Saradnik.findOne({ registrovaniKorisnikId: user._id });

      if (saradnik != null) {
        const token = generateAccessToken(saradnik.registrovaniKorisnikId)
        const refreshToken = generateRefreshToken(saradnik.registrovaniKorisnikId)
        ...
        return res.status(404).json("Traženi saradnik ne postoji");
      }
    }
  }
  catch (err) {
      ...
      if (token) {
        ...
        jwt.verify(token, process.env.TOKEN_KEY, { ignoreExpiration: true },
          async (err, user) => {
            ...
            else (korisnik1.tipKorisnika == "Saradnik")
            {
              const saradnik = await Saradnik.findOne({ registrovaniKorisnikId: korisnik1._id });

              if (saradnik != null) {

                if (saradnik._id.toString() !== req.query.userId) {
                  return res.status(402).json('Nije vaš nalog')
                }

                const noviToken = generateAccessToken(saradnik.registrovaniKorisnikId)
                const refreshToken = generateRefreshToken(saradnik.registrovaniKorisnikId)

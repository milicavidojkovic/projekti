...
export const prikaziSveKvaroveSaradnik = async(req, res) => {
    try 
    {
        const saradnik = await Saradnik.findById(req.params.id);
        console.log(saradnik)
        const kv = await Kvar.find( {saradnikId: saradnik._id} );
        console.log(kv)
        let kvarovi = [];
        for(let i = 0; i < kv.length; i++)
        {
            if(kv[i].status == "Prosleđen saradniku")
            {
                kvarovi.push(kv[i])
            }
        }
        ...
        {
            return res.status(404).json("Nemate nijedan prosleđen kvar");
        }
...
export const prikaziSvePrihvaceneKvaroveSaradnik = async(req, res) => {
    try 
    {
        const saradnik = await Saradnik.findById(req.params.id);
        console.log(saradnik)
        //const kvarovi = await Kvar.find( {status: "Popravka je u toku"}  && {saradnikId: saradnik._id} );
        const kvarovi = await Kvar.find( {saradnikId: saradnik._id} );
        if( kvarovi.length > 0)
        {
            console.log("uslo u if")
            let vrati = [];
            ...
            for (let i = 0; i < kvarovi.length; i++)
            {
                if(kvarovi[i].status == "Popravka je u toku")
                {
                    let kvar = {
                        urgentnost: kvarovi[i].urgentnost,
                        naslov: kvarovi[i]. naslov,
...
//ovo je u stvari dugme prihvati problem, one mogu da stave neki message box:,problem je prihvaćen
//a onda da ide ovo ostalo

//prosledi problem direktoru
...
export const proslediKvarSaradniku = async (req, res)=> {
    try 
    {
        let saradnik = await Saradnik.findById(req.params.saradnikId); 
        let kvar = await Kvar.findById(req.params.kvarId);                    
        
        await kvar.updateOne({ $set: {'status': "Prosleđen saradniku"} } );
        await kvar.updateOne({ saradnikId: req.params.saradnikId } );
        console.log(kvar)
        await saradnik.updateOne({$push: {dobijeniKvarovi: kvar} });

        res.status(200).json("Problem je prosleđen saradniku!");
    }
    catch (err) 
    {
      return res.status(500).json(err);
...
export const prihvatiKvarSaradnik = async(req, res) => {
    try 
    {
        let kvar = await Kvar.findById(req.params.kvarId);
        //console.log(kvar)
        ...
        await kvar.updateOne({$set: {'status': "Popravka je u toku"} });
        await kvar.updateOne({$set: {'saradnikId': saradnik._id} });

        return res.status(200).json("Prihvaćen kvar")
    }
    catch (err) 
    {
        return res.status(500).json(err);
...
export const odbijKvarSaradnik = async(req, res) => {
    try 
    {
        let saradnik = await Saradnik.findById(req.params.saradnikId);   
        let kvar = await Kvar.findById(req.params.kvarId);
        ...
        await kvar.updateOne({$set: {'status': "Prosleđen direktoru"} });
        await kvar.updateOne({$set: {'saradnikId': saradnik.direktorId}})
        await saradnik.updateOne({$pull: {dobijeniKvarovi: {_id: kvar._id}}})

        return res.status(200).json("Kvar odbijen");
